...
def parse_request(sock: socket.socket, buffer: bytes) -> tuple[str, dict, bytes, str]:
    ...
    return request_line, headers, buffer, body.decode('utf-8', errors='replace')


def build_analysis_text(request_line: str, headers: dict, body: str, path: str) -> str:
    transfer_encoding = headers.get('transfer-encoding', '')
    content_length = headers.get('content-length', '')
    return (
        f'backend-mode: secure\n'
        f'path: {path}\n'
...
def handle_client(client_sock: socket.socket, addr):
    ...
    try:
        while True:
            request_line, headers, buffer, body = parse_request(client_sock, buffer)
            path = request_line.split(' ')[1] if ' ' in request_line else '/'
            response_text = build_analysis_text(request_line, headers, body, path)
...
def run_server():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_sock:
        ...
        print(f'Secure backend listening on {HOST}:{PORT}')
        while True:
